...
Item {
	...
	*/
	...
	/*!
		\qmlmethod string Connection::getDeviceVersion()
		\brief Read the ROM version from the target device

		Returns the ROM version string.
	*/
	function getDeviceVersion() {
		return "not implemented with the selected port/connection"
	}
	...
	function commandLineCommandVersion(args) {
		...
		if (version.length > 0) {
			...
			print("Error while requesting ROM version")
		}
	}
